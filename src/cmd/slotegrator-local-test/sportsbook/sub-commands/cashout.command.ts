...
@SubCommand({
  name: 'cashout' as OperationTypes,
  options: {
    isDefault: false,
  },
})
export class SlotegratorSportsbookCashoutCommand extends CommonSettleCommand<
  (typeof data.cashout)[1]
> {
  private readonly cashoutRequestData = data.cashout[1];
  private readonly cashoutSettlementData = data.cashout[2];
  ...
  constructor(
    ...
    super(logger, httpService, runHistoryRepository, data.cashout.at(0)!);
  }
  ...
  async run(
    ...
    this.updateSettlementData({
      requestData: this.cashoutRequestData,
      settlementData: this.cashoutSettlementData,
      bet,
      amount,
    });

    this.cashoutRequestData.request.body.ref_transaction_id =
      targetBetTransactionId!;

    const { response } = await this.runRequests(
      this.cashoutRequestData,
      this.cashoutSettlementData,
      noSettle,
    );

    await this.saveCashout(this.cashoutRequestData.request.body);

    assert.equal(
      response.balance,
      this.cashoutRequestData.response.balance,
      'Cashout response balance differs from the expected balance',
    );
  }
}

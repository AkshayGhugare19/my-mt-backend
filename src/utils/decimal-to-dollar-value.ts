 ...
 */
export const decimalToDollarsValue = <T extends Decimal | undefined | null>(
  value: T,
  precision = 4,
  ...
  const usdPoints = process.env[ENV.USD_POINTS];
  if (!usdPoints) {
    throw new Error('USD_POINTS env variable is not defined');
  }

  return parseFloat(value.div(usdPoints).toFixed(precision));
};
